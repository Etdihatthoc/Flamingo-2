...
def predict(filepath, question, clap_config, inference_kwargs):
    ...
    if '<reasoning>' in output_decoded and '</conclusion>' not in output_decoded:
        print("re-do generation; no conclusion tag found in output: {}".format(output_decoded))

        if '</reasoning>' in output_decoded:
            current_output_decoded = output_decoded.split('</reasoning>')[0]
        else:
            if len(output_decoded) >= 512:
                output_decoded = output_decoded[:512].strip()
            current_output_decoded = output_decoded[:output_decoded.rfind('.')]
        current_output_decoded += '</REASONING>\n<CONCLUSION>'

        new_sample = sample + current_output_decoded

        # Re-do generation
        text = tokenizer(
            new_sample,  # user concatenated text
            max_length=1024,
            padding="longest",
            truncation="only_first",
            return_tensors="pt"
